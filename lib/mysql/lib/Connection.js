...
function handleError(config) {
  this[$originConn] = mysql.createPool(config);
  this[$isAlive] = true;

  this[$originConn].on('error', (err) => {
    console.log(err);
    if (err.code === 'PROTOCOL_CONNECTION_LOST') {
      handleError.call(this, this);
    } else {
      throw err;
    }
  });
...
class Connection {
  ...
  constructor(config, isPoolConn = false) {
    if (config.query) this[$originConn] = config;
    else this[$originConn] = mysql.createConnection(config);
   ...
   */
  get config() {
    return this[$originConn].config;
  }

  /**
   * Orders the connection is in a pool or not.
   */
  get isPoolConnection() {
    return this[$isPoolConn];
  }
   ...
   */
  get threadId() {
    return this[$originConn].threadId;
  }
  ...
  /**
   * Ternimate the connection.
   * This function will ternimate the connection after any query being complete.
   */
  end() {
    return new Promise((resolve, reject) => {
  ...
  query(cmd, params) {
    return new Promise((resolve, reject) => {
      const conn = this[$originConn];
      const args = [cmd];
      const callback = promiseFn(resolve, reject);
   ...
   * Following queries would not be useful until the function commit or rollback called.
   *
